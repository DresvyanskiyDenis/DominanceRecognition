...
def leave_one_instance_out_predictions(ml_model, hyperparameters, dome_labels: dict_data_type):
    # features are already normalized and PCA-transformed
    # create dict idx_to_ids
    combined_dict = dome_labels
...
def main(statistics, normalization: bool, pca: bool, audio_features:str, output_path: str):
    ...
    # create logger
    logger = pd.DataFrame(
        columns=['ml_algorithm', 'hyperparameters', 'dome_least_acc', 'dome_most_acc'])
    ...
    for ml_algorithm in ml_alrogithms.keys():
        ...
        for combination in tqdm(combinations, desc=f"Testing {ml_algorithm}"):
            ...
            current_predictions = leave_one_instance_out_predictions(ml_alg, combination, dome_labels=dome_data)
